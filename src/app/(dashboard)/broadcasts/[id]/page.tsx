...
function FunnelChart({ steps }: { steps: FunnelStep[] }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <h3 className="mb-4 text-sm font-semibold text-foreground">Funnel Analytics</h3>
        ...
        {steps.map((step) => {
          ...
          return (
            <div key={step.label} className="flex items-center gap-3">
              <span className="w-20 shrink-0 text-xs text-muted-foreground font-medium">
                {step.label}
              </span>
              <div className="relative h-7 flex-1 rounded-full bg-secondary border border-border/10">
                <div
                  className={`h-7 rounded-full ${step.color} transition-[width] duration-500`}
                  style={{ width: `${pctOfMax}%` }}
                />
                <span className="absolute inset-0 flex items-center px-3 text-xs font-semibold text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.4)]">
...
export default function BroadcastDetailPage() {
  ...
  if (error || !broadcast) {
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-2">
        <p className="text-sm text-red-650">{error ?? 'Broadcast not found'}</p>
  ...
  const funnelSteps: FunnelStep[] = [
    ...
    { label: 'Replied', value: broadcast.replied_count, color: 'bg-purple-500' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {/* Delete — inline-confirm pattern matches the pipeline-settings
            ...
            funnel inconsistent. */}
        {confirmDelete ? (
          <div className="flex items-center gap-2 rounded-md border border-red-500/20 bg-red-500/10 px-3 py-1.5 text-sm">
            <span className="text-red-600">Delete this broadcast?</span>
            <Button
              variant="outline"
              size="sm"
              onClick={() => setConfirmDelete(false)}
              disabled={deleting}
              className="h-7 border-border bg-transparent text-foreground hover:bg-secondary rounded-xl"
            >
              Cancel
            </Button>
            <Button
              ...
              disabled={deleting}
              className="h-7 bg-destructive text-white hover:bg-destructive/90 rounded-xl"
            >
          ...
          <Button
            ...
            className="border-red-500/25 bg-transparent text-red-650 hover:bg-red-500/10 disabled:opacity-40 rounded-xl"
          >
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-3 bg-secondary/10">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="border-border bg-card">
                <DropdownMenuItem
                  ...
                  className={
                    statusFilter === 'all' ? 'text-primary font-medium' : 'text-foreground'
                  }
                >
                  All statuses
                </DropdownMenuItem>
                ...
                {filteredRecipients.map((recipient) => {
                  ...
                  return (
                    <TableRow key={recipient.id} className="border-border">
                      ...
                      <TableCell className="max-w-xs truncate text-xs text-red-650">
