...
export function ConversationsChart({ series, loading, range, onRangeChange }: ConversationsChartProps) {
  ...
  return (
    <section className="flex h-full flex-col rounded-none border border-border bg-card">
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        ...
        <div className="flex items-center gap-1 rounded-none bg-secondary p-1">
          {[7, 30, 90].map((r) => (
            <button
              key={r}
              type="button"
      ...
      <footer className="flex items-center gap-4 border-t border-border px-5 py-3 text-xs text-muted-foreground">
        <LegendDot color="#1c69d4" label="Incoming" />
        <LegendDot color="#af52de" label="Outgoing" />
      </footer>
    </section>
  )
}
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      >
        ...
        {/* Outgoing polyline (violet -> purple) */}
        <path
          d={outgoingPath}
          fill="none"
          stroke="#af52de"
        ...
        />
        {/* Incoming polyline (blue -> action blue) */}
        <path
          d={incomingPath}
          fill="none"
          stroke="#1c69d4"
        ...
        {hover !== null && (
          <g pointerEvents="none">
            <line
              ...
              strokeDasharray="3 3"
            />
            <circle cx={hoverX} cy={yFor(data[hover.idx].incoming)} r={3.5} fill="#1c69d4" />
            <circle cx={hoverX} cy={yFor(data[hover.idx].outgoing)} r={3.5} fill="#af52de" />
          </g>
        )}
      </svg>
      ...
      {hovered && hover !== null && (
        <div
          className="pointer-events-none absolute top-0 z-10 -translate-x-1/2 rounded-none border border-border bg-card px-3 py-2 text-[11px] shadow-none text-foreground"
          style={{ left: `${hover.tooltipLeftPx}px` }}
        >
          <div className="font-semibold text-foreground">{longDayLabel(hovered.day)}</div>
          <div className="mt-1 flex flex-col gap-0.5">
            <span className="flex items-center gap-1.5 text-[#1c69d4] font-medium">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-[#1c69d4]" />
              {hovered.incoming} incoming
            </span>
            <span className="flex items-center gap-1.5 text-[#af52de] font-medium">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-[#af52de]" />
