...
const KIND_THEME: Record<ActivityKind, KindTheme> = {
  message: { icon: MessageSquare, badge: 'bg-[#1c69d4]/10 text-[#1c69d4]' },
  contact: { icon: UserPlus, badge: 'bg-[#22c55e]/10 text-[#22c55e]' },
  broadcast: { icon: Radio, badge: 'bg-[#f59e0b]/10 text-[#f59e0b]' },
  automation: { icon: Zap, badge: 'bg-[#dc2626]/10 text-[#dc2626]' },
}
  ...
  return (
    <section className="rounded-none border border-border bg-card">
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        ...
        <Link
          href="/inbox"
          className="text-xs font-medium text-primary hover:text-primary-focus"
        >
          View all →
        </Link>
      </header>
        ...
        <>
          <ul className="divide-y divide-border">
            {visible.map((it, i) => {
              ...
              const stripe = i % 2 === 0 ? 'bg-transparent' : 'bg-secondary/40'
              const row = (
                <div className="flex items-center gap-3 px-5 py-2.5">
                  <span
                    className={cn(
                      'flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-none',
                      theme.badge,
                    )}
                  >
              ...
              {PAGE_SIZES.map((size, i) => {
                ...
                return (
                  <button
                    ...
                    disabled={disabled}
                    className={cn(
                      'rounded-none px-2.5 py-1 font-medium tabular-nums transition-colors',
                      pageSize === size
