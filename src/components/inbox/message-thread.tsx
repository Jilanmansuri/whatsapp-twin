...
const STATUS_OPTIONS: { label: string; value: ConversationStatus; color: string }[] = [
  ...
  { label: "Closed", value: "closed", color: "text-gray-400" },
];
...
export function MessageThread({
  ...
  return (
    <div className="flex flex-1 flex-col bg-card">
      ...
      <div className="flex items-center justify-between gap-2 border-b border-border bg-card px-3 py-3 sm:px-4">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-muted-foreground">{contact.phone.split('@')[0]}</p>
          </div>
          ...
          <Badge
            ...
            className={cn(
              ...
              sessionInfo.expired ? "text-red-500 border-red-200 bg-red-50" : "text-primary border-primary/20 bg-primary/5"
            )}
          >
      ...
      {/* Messages Area */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 bg-[#f5f5f7]">
        {loading ? (
            ...
            {messageGroups.map((group) => (
              <div key={group.date}>
                ...
                <div className="mb-4 flex items-center justify-center">
                  <span className="rounded-full bg-white border border-border/60 shadow-xs px-3 py-1 text-[10px] font-medium text-muted-foreground">
      ...
      <MessageComposer
        ...
        lastCustomerMessageText={
          [...messages]
            .reverse()
            .find((m) => m.sender_type === "customer")?.content_text || ''
        }
      />
    </div>
  );
