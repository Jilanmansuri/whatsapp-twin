...
async function sendViaMeta(input: SendInput): Promise<{ whatsapp_message_id: string }> {
  ...
  if (config.provider === 'unofficial') {
    const waMessageId = `unofficial_bot_${Date.now()}`
    ...
    const { error: msgErr } = await db.from('messages').insert({
      ...
      status: 'sending', // 'sending' triggers whatsapp-service
    })

    if (msgErr) {
      throw new Error(`Failed to insert unofficial bot message: ${msgErr.message}`)
    }
